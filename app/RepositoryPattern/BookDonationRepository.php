...
class BookDonationRepository {
    ...
    */
    ...
    public function searchAvailableBookPackages($request): JsonResponse
    {
        $level=$request->level;
        $exchangePoint_id=$request->exchangePoint_id;
        $semester=$request->semester;
        ...
        $availableBookPackages = BookDonation::with(['images' => function ($query)  {
            ...
            ->join('accounts as donorAccount', 'book_donations.donor_id', '=', 'donorAccount.id')
            ->select([
                'book_donations.id',
                'pointAccount.userName as pointName',
                'donorAccount.userName as donorName',
                'book_donations.level',
                'book_donations.semester',
                'exchange_points.location',
            ...
            ->paginate();
        return response()->json(['status'=>'success','data'=>$availableBookPackages]);
    }
    ...
    public function getUndeliveredDonations(int $donor_id): Collection
    {
        return DB::table('book_donations')->where([
            'donor_id'=>$donor_id,
            'status'=>'غير محجوز وليس في النقطة'
            ...
            ->select([
                ...
                'book_donations.semester',
                'accounts.userName',
                //'book_donations.status',
                DB::raw('Date(book_donations.created_at) as date'),
                DB::raw('Time(book_donations.created_at) as time'),
                (DB::raw('residential_quarters.name AS residentialQuarter')),
                (DB::raw('cities.name AS city')),
            ])->get();
    }

    public function getWaitedDonations(int $donor_id): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
    ...
    public function getRejectedDonationsForUser(int $donor_id): Collection
    {
        return DB::table('book_donations')->where('donor_id',$donor_id,)
    ...
    /*
    public function gerReservationByUser_idAndBookDonation_id(int $user_id,int $bookDonation_id)
    {
